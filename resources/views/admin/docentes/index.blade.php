...
@section('content')
    <div class="barra-docentes">
        <div class="buscador-wrap">
            ...
            <input type="text" id="buscador" placeholder="Buscar docente..." oninput="filtrarDocentes()">
        </div>
            ...
            <table class="tabla-docentes">
                ...
                <tbody id="tabla-docentes">
                    @forelse($docentes as $docente)
                        <tr class="fila-docente" data-nombre="{{ strtolower($docente->nombre) }}">
                            <td data-label="Nombre">{{ $docente->nombre }}</td>
                            <td data-label="Acciones" class="col-acciones">
                                <div class="acciones">
                                    <button class="btn-icono-solo btn-editar" onclick="abrirModalEditar('{{ $docente->id }}', {{ json_encode($docente->nombre) }})" title="Editar">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/>
                                        </svg>
                                    </button>
                                    <button class="btn-icono-solo btn-eliminar" onclick="confirmarEliminar('{{ $docente->id }}')" title="Eliminar">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    @empty
                        <tr id="fila-vacia">
                            <td colspan="2" class="fila-vacia-texto">No hay docentes registrados todavía.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <p id="sin-resultados" class="sin-resultados">No se encontraron docentes con ese nombre.</p>

    {{-- Modal: Nuevo / Editar docente --}}
    <div id="modal-docente" class="modal-overlay">
        <div class="modal-tarjeta">
            <h2 id="modal-titulo" class="modal-titulo">Nuevo docente</h2>
            <form id="form-docente" method="POST">
                @csrf
                <input type="hidden" id="metodo-form" name="_method" value="{{ old('_method') }}">
                <input type="hidden" id="input-docente-id" name="docente_id" value="{{ old('docente_id') }}">
    ...
    <style>
        ...
        /* Responsive: tabla se vuelve tarjetas, nombre y acciones en la misma fila */
        @media (max-width: 640px) {
            ...
            .tabla-docentes td.col-acciones {
                flex-shrink: 0;
            }
            .tabla-docentes td::before {
                content: none;
            }
    ...
    <script>
        ...
        function abrirModalNuevo() {
            document.getElementById('modal-titulo').textContent = 'Nuevo docente';
            document.getElementById('input-nombre').value = '';
            document.getElementById('metodo-form').value = '';
            document.getElementById('input-docente-id').value = '';
            document.getElementById('form-docente').action = "{{ url('admin/docentes') }}";
            document.getElementById('modal-docente').style.display = 'flex';
        }

        function abrirModalEditar(id, nombre) {
            document.getElementById('modal-titulo').textContent = 'Editar docente';
            document.getElementById('input-nombre').value = nombre;
            document.getElementById('metodo-form').value = 'PUT';
            document.getElementById('input-docente-id').value = id;
            document.getElementById('form-docente').action = "{{ url('admin/docentes') }}/" + id;
            document.getElementById('modal-docente').style.display = 'flex';
        }
        ...
        function confirmarEliminar(id) {
            Swal.fire({
                ...
                if (resultado.isConfirmed) {
                    ...
                    form.action = "{{ url('admin/docentes') }}/" + id;
                    form.submit();
                }
            });
        }
        ...
        @if ($errors->any())
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const docenteId = document.getElementById('input-docente-id').value;

                document.getElementById('modal-titulo').textContent = esEdicion ? 'Editar docente' : 'Nuevo docente';
                document.getElementById('form-docente').action = esEdicion
                    ? "{{ url('admin/docentes') }}/" + docenteId
                    : "{{ url('admin/docentes') }}";
                document.getElementById('modal-docente').style.display = 'flex';
